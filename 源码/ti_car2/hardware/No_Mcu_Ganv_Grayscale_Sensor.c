...
unsigned int adc_getValue(void)
{
    ...
    //软件触发ADC开始转换
    DL_ADC12_startConversion(GRAY_OUT_INST);

    //如果当前状态 不是 空闲状态
    while (DL_ADC12_getStatus(GRAY_OUT_INST) != DL_ADC12_STATUS_CONVERSION_IDLE );

    //清除触发转换状态
    DL_ADC12_stopConversion(GRAY_OUT_INST);
    //失能ADC转换
    DL_ADC12_disableConversions(GRAY_OUT_INST);

    //获取数据
    gAdcResult = DL_ADC12_getMemResult(GRAY_OUT_INST, GRAY_OUT_ADCMEM_ADC_CH4);
...
void GetDigital(unsigned short *adc_value,unsigned short *Gray_white,unsigned short *Gray_black,unsigned short *result)
{
    for (int i = 0; i < 8; i++) {
        if (adc_value[i] > Gray_white[i]) {
            result[i] = 1;   // 超过白阈值置1（白色）
        } else if (0<adc_value[i] < Gray_black[i]) {
            result[i] = 0;  // 低于黑阈值置0（黑色）
        }
        // 中间灰度值保持原有状态
    }
}
...
/* 函数功能：传感器主任务（无定时器版本）*/
void No_Mcu_Ganv_Sensor_Task_Without_tick(No_MCU_Sensor*sensor,unsigned short *result)
{
    ...
    convertAnalogToDigital(sensor->Analog_value, sensor->Gray_white,sensor->Gray_black,&sensor->Digtal);// 二值化处理
		GetDigital(sensor->Analog_value, sensor->Gray_white,sensor->Gray_black,result);
