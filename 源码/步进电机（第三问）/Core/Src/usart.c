...
/* USER CODE BEGIN 0 */
uint8_t Usart1_Receive_buf[1];  
extern uint8_t Urxbuf[9];
extern int Usart_Compelet;
/* USER CODE END 0 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef*huart) //���ջص�����
{
	...
	{
	uint8_t temp;
	 static uint8_t count,last_data,last_last_data, head_received;
	 temp=Usart1_Receive_buf[0];
		
	if(head_received == 0) {	
			if(last_data == 0xFE && last_last_data == 0xFF) {  // ��⵽֡ͷ
				head_received = 1;
				count = 0;
			}
		}
		if(head_received == 1) { // ��ʼ��������
			Urxbuf[count] = temp;     
			count++;                
			if(count == 9) {  // ����9�ֽڣ�8����+1У�飩
				head_received = 0;
				count = 0;
